...
function ParentsInfoReport() {
  ...
  const rows = useMemo(() => {
    ...
    return students.filter((student) => {
      ...
      const matchesSearch = !q || [student.rollNumber, student.fullName, student.parentName, student.contactNumber]
  ...
  const handleCSV = () => {
    downloadFile([headerRow.join(","), ...dataRows.map((row) => row.join(","))].join("\n"), "parents-info-report.csv", "text/csv");
  };

  const handleExcel = () => {
    downloadFile([headerRow.join("\t"), ...dataRows.map((row) => row.join("\t"))].join("\n"), "parents-info-report.xls", "application/vnd.ms-excel");
  };
        ...
        <div className="table-wrapper">
          <table>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={headerRow.length} className="empty-row">No parent records found</td>
                </tr>
              ) : (
